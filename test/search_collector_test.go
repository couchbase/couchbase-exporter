...
func TestSearchDescribeReturnsAppropriateValuesBasedOnDefaultConfig(t *testing.T) {
	...
	possibleValues = append(possibleValues,
		test.GetDescString(
			defaultConfig.Collectors.Search.Namespace,
			defaultConfig.Collectors.Search.Subsystem,
			"up",
	...
	for _, val := range defaultConfig.Collectors.Search.Metrics {
		possibleValues = append(possibleValues,
			test.GetDescString(
				defaultConfig.Collectors.Search.Namespace,
				defaultConfig.Collectors.Search.Subsystem,
...
func TestSearchDescribeReturnsAppropriateValuesWithNameOverride(t *testing.T) {
	defaultConfig := config.GetDefaultConfig()
	// set up the name overrides
	metrics := make(map[string]objects.MetricInfo)
	now := time.Now()

	for key, val := range defaultConfig.Collectors.Search.Metrics {
		val.NameOverride = fmt.Sprintf("%s-%s", val.Name, now.Format("YYYY-MM-DD"))
		metrics[key] = val
	}

	defaultConfig.Collectors.Search.Metrics = metrics
	mockCtrl := gomock.NewController(t)
...
func TestSearchCollectReturnsDownIfClientReturnsError(t *testing.T) {
	defaultConfig := config.GetDefaultConfig()
	mockCtrl := gomock.NewController(t)
...
func TestSearchCollectReturnsDownIfClientReturnsErrorOnFts(t *testing.T) {
	defaultConfig := config.GetDefaultConfig()
	mockCtrl := gomock.NewController(t)
...
func TestSearchCollectReturnsUpWithNoErrors(t *testing.T) {
	defaultConfig := config.GetDefaultConfig()
	metrics := make(map[string]objects.MetricInfo)
...
func TestSearchCollectReturnsOneOfEachMetricWithCorrectValues(t *testing.T) {
	defaultConfig := config.GetDefaultConfig()
	mockCtrl := gomock.NewController(t)
	...
	for {
		...
		case m := <-c:
			...
			case "cbfts_scrape_duration_seconds":
				gauge, err := test.GetGaugeValue(m)
				assert.Nil(t, err)
				assert.True(t, gauge > 0, fqName)
			default:
				key := test.GetKeyFromFQName(defaultConfig.Collectors.Search, fqName)
				name := defaultConfig.Collectors.Search.Metrics[key].Name

				sampleName := "fts_" + name

				gauge, err := test.GetGaugeValue(m)
				testValue := test.Last(fts.Op.Samples[sampleName])
			...
			count++
		case <-time.After(1 * time.Second):
			if count >= len(defaultConfig.Collectors.Search.Metrics)+2 {
				return
			}
		}
	}
